...
namespace Reality 
{
	...
	public:
		...
		void StartModelLoading(const std::vector<std::string>& modelPaths);
	...
	private:
		...
		int LaunchModelLoadingThreads(const std::vector<std::string>& modelPaths);
		void CreateModel(const string &path);
		void LoadDataIntoModel(string const &path, Model* model);
		void processMaterials(const aiScene *scene, Model* model);
		void processNode(aiNode *node, const aiScene *scene, Model* model);
		void processMesh(aiMesh *mesh, const aiScene *scene, Model* model);
		void loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName, Material* material, const string &directory);
